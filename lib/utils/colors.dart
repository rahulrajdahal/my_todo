import 'package:flutter/material.dart';

Color success = Color(0xff16AD4D);
Color warning = Color(0xffF77307);
Color danger = Color(0xffDB025B);

const Map<int, Color> gray = {
  50: Color(0xffF8FAFC),
  100: Color(0xffF0F4FA),
  200: Color(0xffE1E8F0),
  300: Color(0xffC3D0DD),
  500: Color(0xff596981),
  600: Color(0xff445668),
  700: Color(0xff2D394B),
  800: Color(0xff1C2A3A),
  900: Color(0xff111625),
};
const Map<int, Color> red = {
  50: Color(0xffF8FAFC),
  100: Color(0xffFBEBED),
  200: Color(0xffE1E8F0),
  300: Color(0xffCAD5E0),
  500: Color(0xff596981),
  600: Color(0xffD13145),
  700: Color(0xff304254),
  800: Color(0xff1C2A3A),
  900: Color(0xff111625),
};
const Map<int, Color> blue = {
  50: Color(0xffF8FAFC),
  100: Color(0xffECF0FC),
  200: Color(0xffD9E1F9),
  300: Color(0xffB3C3F3),
  500: Color(0xff4285F4),
  600: Color(0xff4169E1),
  700: Color(0xff304254),
  800: Color(0xff1C2A3A),
  900: Color(0xff111625),
};
const Map<int, Color> green = {
  50: Color(0xffF8FAFC),
  100: Color(0xffECF0FC),
  200: Color(0xffD9E1F9),
  300: Color(0xffB3C3F3),
  500: Color(0x34a85333),
  600: Color(0xff34A853),
  700: Color(0xff304254),
  800: Color(0xff1C2A3A),
  900: Color(0xff111625),
};
